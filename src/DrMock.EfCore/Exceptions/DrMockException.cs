...
namespace DrMock.EfCore.Exceptions
{
    /// <summary>
    /// DrMock Exception
    /// General Exception thrown where Expected Conditions are not met.
    /// </summary>
    public class DrMockException : Exception
    {
        ...
        internal static DrMockException DbSetNotFoundForProperty<T>()
        {
            return DbSetNotFoundForProperty(typeof(T));
        }

        internal static DrMockException DbSetNotFoundForProperty(Type type)
        {
            return new DrMockException($"No DbSet of type {type.Name} was found on the DbContext");
        }

        internal static DrMockException MultipleProperiesForSameType<T>()
        {
            return MultipleProperiesForSameType(typeof(T));
        }

        internal static DrMockException MultipleProperiesForSameType(Type type)
        {
        ...
        internal static DrMockException NonVirtualProperty<T>()
        {
            return NonVirtualProperty(typeof(T));
        }

        internal static DrMockException NonVirtualProperty(Type type)
        {
            return new DrMockException($"DbSet properties need to be set to Virtual. The DbSet for type {type.Name} is not virtual");
        }

        internal static DrMockException CallExpectedNotMade<T>(EfMethod efMethod)
        {
        ...
        internal static DrMockException CallMadeOnBothContextAndSet<T>(EfMethod efMethod)
        {
            if (typeof(IDbContext).GetTypeInfo().IsAssignableFrom(typeof(T).Ge‌​tTypeInfo()))
        ...
        internal static DrMockException CallMadeIncorrectFrequency<T>(EfMethod efMethod, Times timesExpected, int actualFrequency)
        {
